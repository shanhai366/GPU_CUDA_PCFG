...
public:
    ...
    float prob;
       // 新增：计算工作量评估
    size_t EstimateWorkload() const {
        if(content.empty() || max_indices.empty()) return 0;
        
        size_t workload = 1;
        for(size_t i = 0; i < max_indices.size(); ++i) {
            if(i < curr_indices.size()) {
                workload *= (max_indices[i] - curr_indices[i]);
            } else {
                workload *= max_indices[i];
            }
        }
        return workload;
    }
...
public:
    ...
    // 对优先队列的一个PT，生成所有guesses
      void Generate(const PT& pt);  // 确保这是正确的声明
    ...
    vector<string> guesses;
     // 新增批量处理接口
    void ProcessBatch(const std::vector<PT>& pts, size_t batch_size = 1000);
    void GenerateWithCUDA(const PT& pt);


private:
    // 新增GPU处理函数
    void GenerateOnGPU(const PT& pt);
};
